...
// **********************************************************************
void HardwareSerial::config(uint8_t word_length, uint8_t stop_bit, uint8_t party_check)
{
	...
	if ( party_check == STGNSS_UART_NOPARITY
		|| party_check == STGNSS_UART_ODDPARITY
		|| party_check == STGNSS_UART_EVENPARITY) {
		partyCheck = party_check;
	}
}
...
void HardwareSerial::begin(uint32_t baudrate)
{
	...
	// set the serial port
	gnss_uart_init(port_id, valueBaudRate, numWordLength, numStopBit, partyCheck);
